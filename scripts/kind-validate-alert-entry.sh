...
ALERT_PROMPT="${ALERT_PROMPT:-Investigate alert PodCrashLooping for pod crashy in namespace ${SMOKE_NAMESPACE}. Preserve alertname=PodCrashLooping and the original alert-derived target string pod/crashy explicitly. Resolve the target if needed, build a plan, call handoff_active_evidence_batch first with the same incident request and no handoff_token, and if it returns next_action=submit_external_steps then gather the requested bounded peer evidence and call handoff_active_evidence_batch again with handoff_token=<returned handoff_token> and submitted_steps=<typed artifacts for the pending external steps>. If it returns next_action=call_handoff_again, call handoff_active_evidence_batch once more with handoff_token=<returned handoff_token>. Use the latest returned execution_context only for the final render_investigation_report call. Treat get_active_evidence_batch, submit_evidence_step_artifacts, and advance_investigation_runtime as lower-level seams, and treat execute_investigation_step and update_investigation_plan as fallback/debug only. Return Diagnosis, Evidence, Related Data, Limitations, and Recommended next step.}"
